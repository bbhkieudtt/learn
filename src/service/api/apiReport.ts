...
export const apiGetListReport = async () => {
  try {
    return await apiRequest({
      // Phương thức
      method: "GET",
      // endpoint API
      end_point: "Report/get-list",
      // payload được truyền từ giao diện
      // body: payload,
    });
  } catch (e) {
 ...
 /**api lấy danh sách bình luận*/
 export const apiUpdateReport = async (payload: any) => {
  try {
    return await apiRequest({
      // Phương thức
      method: "PUT",
      // endpoint API
      end_point: "Report/update",
      // payload được truyền từ giao diện
      body: payload,
    });
  } catch (e) {
